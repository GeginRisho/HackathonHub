...
export default function ResultsPage() {
  ...
  const loadRankings = async (hackathon: any) => {
    ...
    // Get all teams with evaluations for this hackathon
    ...
    const teamsWithScores = (teams || [])
      ...
      .map(team => ({
        ...team,
        total_score: team.evaluations[0]?.total_score || 0,
        evaluation: team.evaluations[0]
      }))
  ...
  const declareWinners = async () => {
    if (userRole !== 'admin') {
      alert('Only Admins can declare winners.')
      return
    }
  ...
  return (
    <div className="premium-container fade-in">
      <div style={{ marginBottom: '40px' }}>
        <h1 style={{ fontSize: '32px', marginBottom: '8px', fontFamily: 'var(--font-display)' }}>🏆 Results & Leaderboards</h1>
        <p style={{ color: 'var(--text-secondary)' }}>View final evaluation scorecards, winner podiums and team rankings.</p>
      </div>
                  ...
                  {winners.find(w => w.rank === 1) && (
                    <div style={{ textAlign: 'center', width: '150px' }}>
                      ...
                      <div style={{ 
                        ...
                        <span style={{ fontSize: '42px', animation: 'bounce 2s infinite' }}>🥇</span>
                      ...
                      <p style={{ marginTop: '12px', fontWeight: 700, color: '#ffed4e', fontSize: '16px' }}>
